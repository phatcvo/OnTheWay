...
from otw_env.common import utils
from otw_env.common.abstract import AbstractEnv
from otw_env.road.road import RoadNetwork, LineType, StraightLane, CircularLane, AbstractLane
from otw_env.vehicle.kinematics import Vehicle
from controller import ControlledVehicle
from otw_env.road.regulations import RegulatedRoad


class TroadEnv(AbstractEnv):
    ...
    def _make_road(self) -> None:
        ...
        for corner in [-1, 0, 1]: #incomming and exit
            ...
            net.add_lane("o" + str(corner % 4), "ir" + str(corner % 4),
                         StraightLane(start, end, line_types=[s, c], priority=priority, speed_limit=10))

            # Exit
            exit_start = start
            exit_end = deepcopy(end)
            exit_start[is_horizontal] = -exit_start[is_horizontal]
            exit_end[is_horizontal] = -exit_end[is_horizontal]
            net.add_lane("il" + str(corner % 4), "o" + str(corner % 4),
                         StraightLane(exit_end, exit_start, line_types=[n, c], priority=priority, speed_limit=10))

            if corner in [-1 ,0]:
                l_center = rotation @ (np.array([-left_turn_radius + lane_width / 2, left_turn_radius - lane_width / 2]))
                net.add_lane("ir" + str(corner % 4), "il" + str((corner + 1) % 4),
            ...
            if corner == 1:
                inter_start = end
                inter_end = deepcopy(end)
                inter_end[1- is_horizontal] = -inter_end[1- is_horizontal]
                net.add_lane("ir" + str(corner % 4), "il" + str((corner+2) % 4),
                             StraightLane(inter_start, inter_end, line_types=[n ,n], priority=priority, speed_limit=10))
            ...
            if corner == -1:
                ...
                inter_end[1- is_horizontal] = -inter_end[1- is_horizontal]
                net.add_lane("ir" + str(corner % 4), "il" + str((corner+2) % 4),
                             StraightLane(inter_start, inter_end, line_types=[n ,c], priority=priority, speed_limit=10))
    ...
    def _spawn_vehicle(self,
        ...
        route = self.np_random.choice([0,1,3], size=2, replace=False)
