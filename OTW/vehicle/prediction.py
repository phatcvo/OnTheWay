...
from OTW.common.utils import Vector, wrap_to_pi, point_in_rectangle, rotated_rectangles_intersect
from OTW.common.interval import polytope, vector_interval_section, integrator_interval, \
    interval_negative_part, intervals_diff, intervals_product, LPV, interval_absolute_to_local, interval_local_to_absolute
from OTW.road.road import Route, LaneIndex, Road
from OTW.vehicle.controller import LinearVehicle, MDPVehicle
...
class IntervalVehicle(LinearVehicle):
    def __init__(self,
                 ...
                 timer: float = None,
                 theta_a_i: List[List[float]] = None, # The interval of possible acceleration parameters
                 theta_b_i: List[List[float]] = None, # The interval of possible steering parameters
                 data: dict = None) -> None:
    ...
    def observer_step(self, dt: float) -> None:
        ...
        phi_a_i[:, 0] = [0, 0]
        if front_interval:
            phi_a_i[:, 1] = interval_negative_part(intervals_diff(front_interval.speed, v_i))
        ...
        keep_stability = True
        if keep_stability:
            delta_psi = list(map(wrap_to_pi, psi_i - lane_psi))
    ...
    def predictor_step(self, dt: float) -> None:
        ...
        if self.target_lane_index != self.previous_target_lane_index:
            ...
            psi_i = self.interval.heading + target_lane.heading_at(longi_i.mean()) - previous_target_lane.heading_at(longi_i.mean())
            ...
            x_i_local_unrotated = self.longitudinal_lpv.change_coordinates(self.longitudinal_lpv.x_i_t, back=True, interval=True)
            x_i_local_unrotated[:, 0] = longi_i
            new_x_i_t = self.longitudinal_lpv.change_coordinates(x_i_local_unrotated, back=False, interval=True)
    ...
    def predictor_init(self) -> None:
        ...
        if not self.longitudinal_lpv:
            ...
            center = [-self.DISTANCE_WANTED - self.target_speed * self.TIME_WANTED, 0, self.target_speed, self.target_speed]
            noise = 1
            b = np.eye(4)
            d = np.array([[1], [0], [0], [0]])
    ...
    def get_followed_lanes(self, lane_change_model: str = "model", squeeze: bool = True) -> List[LaneIndex]:
        ...
        # lane_change_model: - model: assume that the vehicle will follow the lane of its model behaviour.
        ...
        # squeeze: if True, remove duplicate lanes (at boundaries of the road)
        ...
        if lane_change_model == "model":
            ...
            if _id < len(self.road.network.graph[_from][_to]) - 1 and self.road.network.get_lane((_from, _to, _id + 1)).is_reachable_from(self.position):
    ...
    def partial_observer_step(self, dt: float, alpha: float = 0) -> None:
        ...
        # alpha: ratio of the full interval that defines the boundaries
    ...
    def handle_collisions(self, other: 'RoadObject', dt: float) -> None:
        ...
        # For robust planning, we assume that MDPVehicles collide with the uncertainty set of an IntervalVehicle, which corresponds to worst-case outcome.
        ...
        if not point_in_rectangle(other.position, self.interval.position[0] - self.LENGTH, self.interval.position[1] + self.LENGTH):
            return
        ...
        if rotated_rectangles_intersect((projection, self.LENGTH, self.WIDTH, self.heading),
