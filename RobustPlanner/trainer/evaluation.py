...
class Evaluation(object):
    ...
    def run_batched_episodes(self):
        ...
        for batch, batch_size in enumerate(batch_sizes):
            ...
            workers_params = list(utils.zip_with_singletons(env_config,
                                                      agent_config,
    ...
    def write_logging(self):
        file_infix = '{}.{}'.format(id(self.wrapped_env), os.getpid())
        RobustPlanner.trainer.logger.configure()
        RobustPlanner.trainer.logger.add_file_handler(self.run_directory / self.LOGGING_FILE.format(file_infix))
