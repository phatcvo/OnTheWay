...
from RobustPlanner.agents.control.linear_feedback import LinearFeedbackAgent
from RobustPlanner.common.utils import pos, neg
...
class IntervalFeedback(LinearFeedbackAgent):
    ...
    def synthesize_controller(self, pole_placement=False, ensure_stability=True):
        # pole_placement: use pole placement to synthesize the controller instead
        # ensure_stability: whether we need to check stability when the pole placement method is used
        # whether we found stabilising controls (True if ensure_stability is False)
    ...
    def stability_lmi(self, cA0, cA1, cA2, cB, synthesize_control=True, epsilon=1e-9):
        ...
        # cA0: extended nominal matrix
        # cA1: extended state matrix uncertainty
        # cA2: extended state matrix uncertainty
        # cB: extended control matrix
        # synthesize_control: if true, the controls will be synthesized via the LMI
        # epsilon: accuracy for checking that a matrix is positive definite.
        ...
        return success # whether the controlled system is stable
    ...
    def compute_attraction_basin(self, cB, Gamma, Omega, P, Zp, Zn):
        ...
        # Zp: LMI matrix
        # Zn: LMI matrix
        # An interval asymptotically containing \xi, under the closed-loop dynamics tested in the stability_lmi.
