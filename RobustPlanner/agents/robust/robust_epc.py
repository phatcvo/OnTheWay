...
from RobustPlanner.common.factory import load_agent, safe_deepcopy_env
...
class RobustEPCAgent(AbstractAgent):
    ...
    def record(self, observation, action, reward, next_observation, done, info):
        # observation: state x_t and derivative \dot{x}_t at time t
        # action: action a_t performed
        # reward: reward r_t obtained
        # next_observation: next state and derivative at time t+dt
    ...
    def record_transition(self, state, derivative, control):
        # state: state x_t at time t
        # derivative: state derivative dot x_t at time t
        # control: control u_t at time t
    ...
    # Compute a confidence ellipsoid over the parameter theta, under a sub-Gaussian noise assumption.
    def ellipsoid(self):
        """
        ...
        return theta_n_lambda, g_n_lambda, beta_n # estimated theta, Gramian matrix G_N_lambda, and radius beta_N_lambda
    ...
    def polytope(self):
        ...
        return a0, da # nominal matrix A0, list of vertices dA, such that A(theta) = A0 + alpha^T dA.
    ...
    def robustify_env(self):
        ...
        """
        a0, da = self.polytope()
        from OTW.common.interval import LPV
        ...
        return robust_env # the robust version of the environment.
