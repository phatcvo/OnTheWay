...
# from RobustPlanner.agents.robust.graphics.robust_epc_graphics import RobustEPCGraphics, confidence_ellipse
# from RobustPlanner.agents.robust.robust_epc import NominalEPCAgent

matplotlib.rc('text', usetex=False)
import seaborn as sns


class ConstrainedEPCGraphics(agents.robust.graphics.robust_epc_graphics.RobustEPCGraphics):
    ...
    def display_attraction_basin(cls, env, agent, surface, alpha=50):
        if "OTW" not in env.unwrapped.__module__:
            return
        try:
    ...
    # Plot the hull of all ellipsoids. If a threshold beta and corresponding mixture is provided, plot them.
    @classmethod
    def plot_ellipsoid(cls, ellipsoids, true_theta, config, title="", figsize=(8, 6), save_to=None):
        ...
        for ellipsoid in ellipsoids[:20:5] + ellipsoids[20:-1:20]:
            agents.robust.graphics.robust_epc_graphics.confidence_ellipse(ellipsoid, ax, facecolor=(1, 0.3, 0.3, 0.1), edgecolor="black", linewidth=0.5, label=None)
        agents.robust.graphics.robust_epc_graphics.confidence_ellipse(ellipsoids[-1], ax, facecolor=(1, 0.3, 0.3, 0.1), edgecolor='red', label=r"$\hat{\Theta}$")
