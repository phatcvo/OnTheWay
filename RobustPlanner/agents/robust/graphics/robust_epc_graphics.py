...
class RobustEPCGraphics(agents.robust.graphics.robust_graphics.IntervalRobustPlannerGraphics):
    SAVE_IMAGES = None
    ...
    def plot_ellipsoid(cls, ellipsoids, true_theta, config, title="", figsize=(8, 6), save_to=None):
        ...
        for ellipsoid in ellipsoids[20:-1:20]:
            confidence_ellipse(ellipsoid, ax, facecolor=(1, 0.3, 0.3, 0.1), edgecolor="black", linewidth=0.5, label=None)
        confidence_ellipse(ellipsoids[-1], ax, facecolor=(1, 0.3, 0.3, 0.1), edgecolor='red', label=r"$\mathcal{C}_{[N],\delta}$")
...
def confidence_ellipse(ellipsoid, ax, facecolor="none", **kwargs):
    ...
    ellipsoid = Ellipse((0, 0), width=ell_radius_x * 2, height=ell_radius_y * 2, facecolor=facecolor, **kwargs)
