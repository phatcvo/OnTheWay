...
# from RobustPlanner.common.factory import preprocess_env
# from RobustPlanner.agents.tree_search.graphics import TreeGraphics


class DiscreteRobustPlannerGraphics(agents.tree_search.graphics.TreeGraphics):
    @classmethod
    def display(cls, agent, agent_surface, sim_surface):
        plan = agent.planner.get_plan()
        for env in [common.factory.preprocess_env(agent.true_env, preprocessors) for preprocessors in agent.config["models"]]:
