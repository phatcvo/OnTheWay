...
from RobustPlanner.common.utils import remap, constrain

# Graphical visualization of a tree-search based agent. (draw tree-search)
class TreeGraphics(object):
    ...
    @classmethod
    def display(cls, agent, surface, max_depth=4): #agent: the agent to be displayed, surface: the pygame surface on which the agent is displayed
        ...
        if not surface:
            return
        import pygame
        cell_size = (surface.get_height()/1.1 // (max_depth + 1), surface.get_width())
        pygame.draw.rect(surface, cls.WHITE, (0, 0, surface.get_width(), surface.get_height()), 0)
        cls.display_node(agent.planner.root, agent.env.action_space, surface, (0, 0), cell_size,config=agent.planner.config, depth=0, selected=True)
        act = []
        ...
        surface.blit(text1, (10, 100-50))

        font = pygame.font.Font(None, 15)
        text = font.render('=>'.join(map(str, act)), 1, (10, 10, 10), (255, 255, 255))
        surface.blit(text, (10, 100-20))

    # Display an MCTS node at a given position on a surface.
    @classmethod
    def display_node(cls, node, action_space, surface, origin, size, config=0, depth=0, selected=False):
        """
        ...
        if selected:
            pygame.draw.rect(surface, cls.BLUE, (origin[1], origin[0], size[1], size[0]), 3)
        ...
        for i, action in enumerate(node.children):
            ...
            cls.display_node(node.children[action], action_space, surface,
                             ...
                             (size[0], size[1]/num_cells), depth=depth+1, config=config, selected=action_selected)
    ...
    def draw_node(cls, node, surface, origin, size, config):
        ...
        pygame.draw.rect(surface, color, (origin[1], origin[0], size[1], size[0]), 0)
    ...
    def display_text(cls, node, surface, origin, config):
        ...
        surface.blit(text, (10, origin[0] + 50))

        text1 = "node(value/num)"
        text1 = font.render(text1, 1, (10, 10, 10), (255, 255, 255))
        surface.blit(text1, (10, surface.get_height()-790))
