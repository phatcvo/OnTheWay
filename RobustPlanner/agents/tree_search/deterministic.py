...
class DeterministicNode(Node):
    def __init__(self, parent, planner, state=None, depth=0):
        ...
        self.count = 1
        self.done = False
        # print("Initialized DeterministicNode")
    ...
    def update(self, reward, done, observation=None):
        ...
        if isinstance(done, np.ndarray):
            idx = np.where(done)
            next_value = self.value_lower[idx] + self.planner.config["terminal_reward"] * (gamma ** self.depth) / (1 - gamma)
            self.value_lower[idx] = next_value
            self.value_upper[idx] = next_value
        elif done:
            self.value_lower = self.value_upper = self.value_lower + self.planner.config["terminal_reward"] * (gamma ** self.depth) / (1 - gamma)
