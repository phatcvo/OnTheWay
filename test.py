...
from RobustPlanner import trainer, agents
# .trainer import logger
...
def evaluate(environment_config, agent_config, options):
    ...
    """
    trainer.logger.configure(LOGGING_CONFIG)
    if options['--verbose']:
        trainer.logger.configure(VERBOSE_CONFIG)
    env = agents.common.factory.load_environment(environment_config)
    agent = agents.common.factory.load_agent(agent_config, env)
    run_directory = None
    ...
    evaluation = trainer.evaluation.Evaluation(env, agent, run_directory=run_directory,
                            ...
                            display_rewards= options['--no-display']) # display_rewards=not options['--no-display']
    # RewardViewer()
    # print("===============================", rewards)
    if options['--train']:
        evaluation.train()
...
def benchmark(options):
    ...
    benchmark_filename = os.path.join(trainer.evaluation.Evaluation.OUTPUT_FOLDER, '{}_{}.{}.json'.format(
        BENCHMARK_FILE, datetime.datetime.now().strftime('%Y%m%d-%H%M%S'), os.getpid()))
    with open(benchmark_filename, 'w') as f:
        json.dump(results, f, sort_keys=True, indent=4)
        gym.trainer.logger.info('Benchmark done. Summary written in: {}'.format(benchmark_filename))
